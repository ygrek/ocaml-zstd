...
let check_size_offset size offset len =
  ...
  let size = match size with None -> len | Some s when s < 0 -> raise (Invalid_argument "negative size") | Some s -> s in
  if size + offset > len then raise (Invalid_argument "given size is bigger than string len");
  size, offset
